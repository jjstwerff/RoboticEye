...
void test(int test, const char *message) {
  if (!test) {
    printf("%s\n", message);
    exit(1);
  }
...
static void registry_add_object(void *data, struct wl_registry *registry, uint32_t name, const char *interface,
                                uint32_t version) {
  data = data; // lame measure to prevent warnings
  version = version;
...
static void shell_surface_configure(void *data, struct wl_shell_surface *shell_surface, uint32_t edges, int32_t width,
                                    int32_t height) {
  shell_surface = shell_surface; // lame measure to prevent warnings
  edges = edges;
...
static struct wl_shell_surface_listener shell_surface_listener = {&shell_surface_ping, &shell_surface_configure,
...
int main() {
  ...
  test(display != NULL, "Could not initialize wayland");
